...
ImageProcessor.prototype.processAvatar = function(path, destPath) {
	var that = this
	var info = {}
...
ImageProcessor.prototype.handleAvatarUpload = function(file, destDir) {
	var that = this
	...
	.then(function(oinfo) {
		info.original = oinfo
		return that.createScaledAvatar(normalizedPath)
	})
	// analyse and store
	.then(function(scaledPath) {
		return that.processAvatar(scaledPath, scaledName)
...
ImageProcessor.prototype.createScaledAvatar = function(path) {
	var dfd = when.defer()
