...
ImageProcessor.prototype.handleUpload = function(file, destDir)
{
	...
	var thumbName = fsPath.join(destDir, name + '-thumb' + ext);
	var scaledName = fsPath.join(destDir, name + '-scaled' + ext);
	var scaledThumbName = fsPath.join(destDir, name + '-scaled-thumb' + ext);
	...
	.then(function(info)
	{
		...
		{
			return that.createPowerOfTwoImage(file.path, info.original)
			.then(function(p2)
			{
...
{
	console.log('ImageProcessor.createPowerOfTwoImage', path, info.width, info.height);
	var that = this;
	var dfd = when.defer();
...
ImageProcessor.prototype.generateThumbnail = function(path, thumbPath)
{
	...
	.write(tempPath, function(err)
	{
		...
		.then(function(info)
		{
			...
			return that.storeImage(tempPath, thumbPath);
		})
		.then(function(url)
		{
			thumbInfo.url = url;
