...
function guid() {
	return hashids.encode(Math.floor(Math.random() * Number.MAX_SAFE_INTEGER))
}
...
EditorChannelServer.prototype.tail = function(channelName, client, lastEditSeen) {
	var that = this

	console.log('tail', channelName, client.id, lastEditSeen)
...
EditorChannelServer.prototype.joinChannel = function(channelName, client, message) {
	...
	this.tail(channelName, client, message.lastEditSeen)
...
EditorChannelServer.prototype.sendToChannel = function(channelName, message, exceptId) {
	...
	this.channels[channelName].map(function(client) {
		...
		console.log('sendToChannel', message.actionType || message.kind, message.channel)
