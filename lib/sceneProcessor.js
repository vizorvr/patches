...
SceneProcessor.prototype.handleUpload = function(file, sceneFolder) {
	...
	.then(function() {
		...
		if (['.obj', '.js', '.json'].indexOf(extname) === -1)
...
SceneProcessor.prototype.processZipUpload = function(file, sceneFolder) {
	...
	var mainFileExtensions = ['.obj', '.json', '.js']
	var mainFile
	...
	fs.createReadStream(file.path)
		...
		.on('close', function() {
			...
			when.map(filesToProcess, function(entry) {
				var destPath = sceneFolder + '/' + fsPath.basename(entry.path)

				return that.processUploadedFile(entry, destPath)
				.then(function() {
					files.push(destPath)
				})
			})
