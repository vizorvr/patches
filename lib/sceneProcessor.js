...
var mainFileExtensions = ['.obj', '.js', '.json']
...
SceneProcessor.prototype.validate = function(file) {
	...
	if (extname === '.zip') {
		yauzl.open(file.path, function(err, zip) {
			...
			.on('close', function() {
				console.log('jsonFound', jsonFound)
...
SceneProcessor.prototype.processUploadedFile = function(stream, destPath) {
	...
	console.log('processUploadedFile: pass', destPath)
...
SceneProcessor.prototype.processZipUpload = function(file, sceneFolder) {
	...
	var that = this
	var files = []
	var allowedExtensions = ['.obj', '.jpg', '.png', '.tga', '.json', '.js', '.mtl', '.dds']
	var mainFile
	...
	yauzl.open(file.path, function(err, zip) {
		...
		zip.on('entry', function(entry) {
			...
			console.log('processZipUpload: pass', entry.path)
		...
		.on('error', function(err) {
			console.log('processZipUpload error', err)
			dfd.reject(err);
		})
		.on('close', function() {
		...
		.on('end', function() {
			console.log('processZipUpload: end', filesToProcess.length)
